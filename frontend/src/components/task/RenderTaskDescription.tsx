...
import { DESCRIPTION_SHORT_LENGTH, URL_REGEX } from "../constants/constants";
...
export const RenderTaskDescription = memo(
  ({ task }: RenderTaskDescriptionProps): JSX.Element | null => {
    const { expandedTasks, toggleShowMore, highlightMatchingText } = useTasks();
    ...
    const descriptionWithLinks = parts.map((part, index) => {
      if (index % 2 === 0) {
        ...
        try {
          ...
          const mapping = domainMappings.find(({ regex }) =>
            domain.match(regex)
          );
          icon = mapping ? mapping.icon : <Link />; // Default to Link icon
          domain =
            mapping && mapping.domainName
              ? mapping.domainName
          ...
          <Tooltip
            title={
              <>
                ...
                {part.match(domainMappings[0].regex) && youtubeId(part) && (
                  <YouTubeThumbnail>
                    <img
